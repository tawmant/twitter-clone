...
import Tweets from "../../components/HomeComponents/Tweets/Tweets";
import axios from "axios";
import { ITweetItem, TweetsActionTypes } from "../../types/ITweets";
...
const HomePage: FC = () => {
  ...
  const tweetHandler = async () => {
    fetch("https://twitter-api-v2.herokuapp.com/api/tweet/list/", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify(tweetValue),
    })
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
  ...
  return (
    <div className="app__content col-6">
      <div className="app__title py-3 px-4 d-flex justify-content-between align-items-center">
        <h1>Home</h1>
        <span className="icon">
      ...
      <div className="app__post-news pt-2 pb-4 px-4">
        <img
          className="app__post-left post__img"
          src="https://pbs.twimg.com/profile_images/1441611208176455681/lA7WO8i1_normal.png"
          alt=""
        />
        <div className="app__post-right">
          <textarea
            className="app__post-input"
            onChange={(e) =>
              setTweetValue({ ...tweetValue, text: e.target.value })
            }
            value={tweetValue.text}
            placeholder="What's happening?"
          />
          <div className="line-post"></div>
          <div className="post-right-bottom">
            <ul className="block__add-any">
              <li className="icon">
                <label htmlFor="post-file">
                  <input
                    onChange={
                      (e) =>
                        setTweetValue((prev: any) => ({
                          ...prev,
                          //@ts-ignore
                      ...
                      // setTweetValue({
                      //   ...tweetValue,
                      //   images: [...tweetValue.images, e.target.file[0]],
                      // })
                    }
                    type="file"
                    id="post-file"
            ...
            <button onClick={tweetHandler} className="app__tweet">
              Tweet
            </button>
          </div>
        </div>
