...
const Tweets: FC = () => {
  ...
  const { tweets, loading, error } = useTypedSelector(
    ({ tweetsReducer }) => tweetsReducer
  );
  const [offset, setOffset] = useState<number>(0);
  const tweetsRef = useRef<HTMLUListElement>(null);
  ...
  return (
    <ul ref={tweetsRef}>
      {tweets.map((tweet: ITweetItem) => (
        <TweetItem key={tweet.id} dataTweet={tweet} token={auth.token} />
      ))}
    </ul>
  );
};
